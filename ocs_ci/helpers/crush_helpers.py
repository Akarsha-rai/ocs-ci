...
important for stretch cluster configurations.
"""
...
def get_osd_crush_weights() -> Dict[int, float]:
    ...
    Returns:
        dict: Dictionary mapping OSD IDs to their CRUSH weights
              Example: {0: 1.81998, 1: 1.81998, 2: 1.81998}

    Raises:
        CommandFailed: If unable to get OSD information from Ceph
...
def get_zone_crush_weights(zones: Optional[List[str]] = None) -> Dict[str, float]:
    """
    Get aggregated CRUSH weights for each zone in the cluster.

    Args:
        zones (list, optional): List of zone labels to check. If None, uses
                               constants.DATA_ZONE_LABELS

    Returns:
        dict: Dictionary mapping zone names to their total CRUSH weights
              Example: {'zone-a': 5.45994, 'zone-b': 5.45994}

    Raises:
        CommandFailed: If unable to get zone weights from Ceph
    ...
    try:
        for zone in zones:
            # Get the zone weight from ceph osd tree output
            # The command extracts the weight column (field 2) for the specified zone
            command = f"ceph osd tree | grep 'zone {zone}' | awk '{{print $2}}'"
            weight_output = ceph_tools_pod.exec_sh_cmd_on_pod(command=command)
            
            # Parse the weight value - handle both string and list returns
            if isinstance(weight_output, list):
                weight_str = str(weight_output[0]).strip() if weight_output else ""
            else:
                weight_str = str(weight_output).strip()
            if weight_str:
                zone_weights[zone] = float(weight_str)
                logger.info(f"Zone '{zone}' CRUSH weight: {zone_weights[zone]}")
            else:
...
def calculate_weight_difference_percentage(weight1: float, weight2: float) -> float:
    ...
    Args:
        weight1 (float): First CRUSH weight
        weight2 (float): Second CRUSH weight

    Returns:
        float: Percentage difference between the weights
    ...
    logger.debug(
        f"Weight difference: {weight1} vs {weight2} = {percentage:.2f}%"
    )
    return percentage
...
def verify_zone_weight_balance(
    ...
    This is critical for stretch cluster configurations where zone weight
    balance is required for proper operation.

    Args:
        zones (list, optional): List of zone labels to check. If None, uses
                               constants.DATA_ZONE_LABELS
        tolerance_percentage (float): Maximum allowed percentage difference
                                     between zone weights (default: 1.0%)

    Returns:
        tuple: (is_balanced, zone_weights, message)
            - is_balanced (bool): True if zones are balanced within tolerance
            - zone_weights (dict): Dictionary of zone weights
            - message (str): Descriptive message about the balance status

    Example:
        >>> is_balanced, weights, msg = verify_zone_weight_balance()
        >>> if not is_balanced:
        ...     logger.error(f"Zone weights unbalanced: {msg}")
    ...
    try:
        ...
        # Get all zone weights as a list
        ...
        # Calculate max difference percentage between any two zones
        ...
        if is_balanced:
            ...
            message = (
                ...
                f"Difference: {max_diff_percentage:.2f}% (tolerance: {tolerance_percentage}%) "
...
def get_crush_tree() -> Dict:
    """
    Get the complete CRUSH tree structure from Ceph.

    Returns:
        dict: CRUSH tree structure as a dictionary

    Raises:
        CommandFailed: If unable to get CRUSH tree from Ceph
    ...
    try:
        crush_tree_result = ceph_tools_pod.exec_ceph_cmd(ceph_cmd="ceph osd tree")
        # Convert to dict if it's a list or other type
        if isinstance(crush_tree_result, dict):
            crush_tree = crush_tree_result
        else:
            # If it's not a dict, try to parse it
            crush_tree = dict(crush_tree_result) if crush_tree_result else {}
        
        logger.debug(f"CRUSH tree retrieved with {len(crush_tree.get('nodes', []))} nodes")
        return crush_tree
...
def get_osds_in_zone(zone: str) -> List[int]:
    ...
    Raises:
        CommandFailed: If unable to get OSD information
    ...
    try:
        ...
        # Find the zone node in the CRUSH tree
        zone_node = None
        ...
        # Recursively find all OSDs under this zone
        ...
        # Start recursive search from zone node
...
def verify_stretch_mode_enabled() -> bool:
    ...
    try:
        # Check mon dump for stretch mode configuration
        mon_dump = ceph_tools_pod.exec_ceph_cmd(ceph_cmd="ceph mon dump")
        mon_dump_str = json.dumps(mon_dump)
        
        is_stretch_enabled = "stretch" in mon_dump_str.lower()
...
def log_crush_weight_details(zones: Optional[List[str]] = None):
    ...
    Args:
        zones (list, optional): List of zones to analyze. If None, uses
                               constants.DATA_ZONE_LABELS
